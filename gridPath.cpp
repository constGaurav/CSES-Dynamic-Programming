...
ll gridPath(ll n, ll m, vector<vector<char>> &grid, vector<vector<ll>> &dp){
	...
	if(dp[n][m] != -1) 
		return dp[n][m];
	ll up = gridPath(n-1,m,grid,dp);
	ll left = gridPath(n,m-1,grid,dp);
